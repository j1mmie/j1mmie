...
Regarding the annoying ship issue, the problem was that some of the scripts used by Super Tiled2Unity were Editor-only scripts.

The solution was to package each Tiled2Unity map in a prefab. I wrote a script that does this, and modified ShipManager to understand the new format. The script also ensures the map is in Addressables with the right group and key.

Next issue that's been bugging me: IPhone pinch gesture stuttering.

When you pinch or stretch on the IPhone, the camera stutters before you're able to control zooming smoothly. Today I verified that Android does not have this issue. I discovered that IPhone has this issue because there is some drag threshold. The first finger does not register as moving until some duration and/or distance minimum is cleared. Then suddenly, it jumps to the finger's current position. That's the first stutter. The second finger behaves the same way. That's the second stutter.

Things I tried:

1. Changing pixelDragTheshold on the Event System. No effect
2. Predicting drags with IInitializePotentialDragHandler and setting useDragThreshold to false. No effect.
3. Using Unity's new Input System, in hopes that I could get unfiltered Touch data. After hours of implementing, I learned it has the same flaw.

So, this issue seems to be deeper than the layer of Unity that I'm working within. It might be a more general iOS problem.

Next ideas:
1. Maybe useDragThreshold should be true and I should ensure that the Event System has a pixelDragTheshold of 0?
2. Smooth that last N pinch deltas
   1. Bonus points: Smooth the last N pinch deltas, where N starts subtracting by 1 each frame after the first move is detected. This would make it so the first stutter (for each finger) gets smoothed, but smoothing eventually catches up to realtime.

## September 9, 2024

Idea for game architecture:

```
Renderer
Stage
Mediators
  Interact directly with stage. Trigger events
Netcode
  Receive network messages (notifications). Send to appropriate Mediator
Comms
  Network messages, serialization, deserialization, etc.
  .
 / \
  |
 \ /
...
I've upgraded us to Unity 6 because staying on 2021 (or 2022) would've kept us on an old licensing model. The licensing model, ridiculously, got rid of Unity Plus ($100 / seat / yr), and would require us to pay for Pro licenses *just* to remove the Unity splash logo. For 3 seats, that's $6,000 a year. Just to remove the fucking splash logo.

I talked them down to $3,000 a year, and even removing a seat would've gotten us to $2,000. That's still way too fucking much. I tried to get down to $300 - the price of three Unity Plus seats. They would not accept.

So, now we're on Unity 6 and they'll get nothing from us.

However, upgrading to Unity 6 was not without its problems:

### Time requirement

It took about a day to do the initial upgrade. Honestly, I was pretty happy to that. But then to address nuanced bugs and release it, it's taken about a week of my time.

### UniWebView stopped working

This was resolved by a simple upgrade to the UniWebView plugin. Didn't take much time to realize, but then the Unity Asset Store was not working for me in Unity 6. I think I solved this by restarting Unity, or re-authing, or something. That was annoying.

### Google Play Games Services incompatibility

The Google Play Games Services plugin just stopped working. Specifically, the Android External Dependency Resolver stopped working. I think.

Took another day to figure this out. I found a workaround project on Github that modernizes the Google Play Games Services plugin and moves it to UPM. Located here: https://github.com/Thaina/play-games-plugin-for-unity.git

Eventually I would like to move off of this, since I don't know it's author or changes well.

### Unity Cloud Build fails to build AAB files

Unity Cloud Build started failing to build Google Play AAB files, delaying our deployment on Android (and thus, all platforms).

Open thread about it here:

https://discussions.unity.com/t/builld-fails-when-trying-to-build-app-bundles-instead-of-apk/947493/6

I (magically) discovered a workaround, implemented in the PreExport method in CloudBuildHelper.cs

### Surprise deprecation of support for Windows 7 and 8

This one is technically my fault. But, it would've been nice if there were some warnings when I did the upgrade. With all the other bullshit involved in the transition, it wasn't something I had top of mind.

### AssetReferenceAtlasedSprite unusable in Inspector UI

AssetReferenceAtlasedSprite broke when I upgrade the Addressables framework. It fails to render properly in the Inspector, and spits out a ton of errors to the console. Looking at the Unity code, to me it seems like the mistake is *very* obvious. I swear some of the code they release at Unity... absolutely laughable.

Bug report here: https://unity3d.atlassian.net/servicedesk/customer/portal/2/IN-87971

### No way to convert SpriteAtlas v1 to SpriteAtlas v2?

Haven't found an obvious way to do this. I thought the upgrade was supposed to handle it automatically.

### MAJOR BUG: Rendering completely broken for some Android devices (Adreno video cards)

Unity's new render graph is completely broken for some Android devices. The screen is completely black. This was not obvious to use upon release because we don't have affected devices. After deploy though, users started reporting the issue.

Some Android devices only support a Depth Stencil Format of D24_UNorm_S8_UInt. Unity's new render graph seems to use Render Textures, and the default Depth Stencil Format is D32_SFloat. This is known to cause issues on some Android devices, and yet somehow Unity forgot to include Android in the list of D24 devices. It is a major regression.

After days of conversing with Unity, they shared a fix. The broken code is as simple as this:
...
The solution is to simple add UNITY_ANDROID to the condition. Unfortunately, editing Unity packages is a nightmare.

A really, really, REALLY bad idea is to simply set the `UNITY_EMBEDDED_LINUX` #define in Player Settings. The reason this isn't a great idea is because I have no clue what other logic is dependent on this #define.

However, I searched where I could, and tested on what devices I had available. I seems that defining `UNITY_EMBEDDED_LINUX` does not break anything. I've deployed this fix to production. Users are happy. Holy fucking shit.
...
- Run migrations (`slipadmin PROD sync-exchange-rates`)
- Test and deploy `servicePublishStaticData` function well before deploy
- Run `publish-static-data` with the new static data collection (`currencyExchangeRates`). Should have no effect on old clients
-
