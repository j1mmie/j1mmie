...
Learning: Get an access token for a service account:

```
gcloud auth print-access-token --impersonate-service-account=SERVICE_ACCOUNT_NAME@PROJECT_NAME.iam.gserviceaccount.com
```

More info can be found in service-account JSON in ~/.slipstream-PROD-storage
