...
1. Deploy an update for `steamConsumeInventoryItems` that does no create the `steamUserId` field and instead saves a `SteamConsumeReport`
2. Migration: Delete all existing `steamUserId` fields using the new admin command `remove-all-steam-user-id-fields`
3. Deploy changes to the auth server to support Steam Sign In
...
2. Build Android app with self-managed key signing (Slipstream Android Keystore). Passwords available in 1Password
3. Go crazy
